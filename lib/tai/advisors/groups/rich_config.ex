defmodule Tai.Advisors.Groups.RichConfig do
  ...
  defp parse_item({k, {{venue_id, product_symbol, account_id}, :fee}}, acc, provider) do
    fee = provider.fees |> find_fee(venue_id, product_symbol, account_id)
    acc |> Map.put(k, fee)
  end
  ...
  defp find_fee(fees, venue_id, symbol, account_id) do
    fees
    |> Enum.find(fn f ->
      f.venue_id == venue_id && f.symbol == symbol && f.account_id == account_id
    end)
  end
end
