defmodule Examples.PingPong.ManageQuoteChange do
  ...
  def manage_entry_order(
        {:ok, _},
        %NewAdvisor.State{store: %{entry_order: %Order{status: :open} = entry_order}} = state
      ) do
    {:ok, market_quote} =
  ...
  def manage_entry_order(
        {:ok, _},
        %NewAdvisor.State{store: %{entry_order: %Order{}}} = state
      ) do
    {:ok, state.store}
  end

  def manage_entry_order({:ok, market_quote}, state) do
    advisor_id = NewAdvisor.process_name(state.fleet_id, state.advisor_id)
