defmodule Tai.NewOrders.GetTest do
  ...
  test ".get_by_client_id/1 returns the order when found and nil otherwise" do
    ...
    assert Tai.NewOrders.get_by_client_id(order_1.client_id) == order_1
    assert Tai.NewOrders.get_by_client_id(Ecto.UUID.generate()) == nil
  end
  ...
  test ".get_by_client_ids/1 returns orders matching the given client ids" do
    ...
    matching_orders = Tai.NewOrders.get_by_client_ids([order_1.client_id, order_2.client_id])
    assert Enum.member?(matching_orders, order_1)
    assert Enum.member?(matching_orders, order_2)

    assert Tai.NewOrders.get_by_client_ids([Ecto.UUID.generate()]) == []
  end
end
