defmodule Tai.NewOrders.AmendBulkTest do
  ...
  |> Enum.each(fn side ->
    ...
    test "#{side} changes the price & qty of each order on the venue and locally records that they were accepte" do
      ...
      Mocks.Responses.NewOrders.GoodTillCancel.amend_bulk_accepted([{open_order, %{price: @amend_price, qty: @amend_qty}}])
      ...
      assert [
        {:ok, returned_order},
        # {:error, amend_error}
      ] = NewOrders.amend_bulk(amend_set)
    ...
    test "#{side} records the error when it can't be canceled on the venue" do
      ...
      assert [{:ok, %NewOrders.Order{status: :pending_amend}}] = NewOrders.amend_bulk(amend_set)
    ...
    test "#{side} records a failed transition and doesn't execute the callback when the adapter returns an invalid response" do
      {:ok, open_order} = create_open_order(%{side: @side})
      Mocks.Responses.NewOrders.GoodTillCancel.amend_bulk_accepted(
        [{open_order, %{price: @amend_price, qty: @amend_qty}}],
        [%{venue_timestamp: "invalid date"}]
      )

      assert [{:ok, _}] = NewOrders.amend_bulk([{open_order, %{price: @amend_price, qty: @amend_qty}}])
    ...
    test "#{side} records an error when raised by the adapter" do
      ...
      Mocks.Responses.NewOrders.Error.amend_bulk_raise(
    ...
    test "#{side} returns an error and records a failed transition for orders with an invalid status for pend amend" do
      ...
      assert [{:error, reason}] = NewOrders.amend_bulk([{filled_order, %{price: @amend_price, qty: @amend_qty}}])
