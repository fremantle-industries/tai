defmodule Tai.Orders.AmendTest do
  ...
  |> Enum.each(fn side ->
    ...
    test "#{side} changes the price & qty on the venue and locally records that it was accepted" do
      ...
      Mocks.Responses.Orders.GoodTillCancel.amend_accepted(@venue_order_id, %{
        price: @amend_price,
        qty: @amend_qty
      })

      assert {:ok, returned_order} =
               Orders.amend(open_order, %{price: @amend_price, qty: @amend_qty})
    ...
    test "#{side} records the error when it can't be canceled on the venue" do
      ...
      assert {:ok, %Orders.Order{status: :pending_amend}} =
               Orders.amend(open_order, %{price: @amend_price, qty: @amend_qty})
    ...
    test "#{side} records a failed transition and doesn't execute the callback when the adapter returns an invalid response" do
      ...
      Mocks.Responses.Orders.GoodTillCancel.amend_accepted(
        @venue_order_id,
        %{price: @amend_price, qty: @amend_qty},
        %{venue_timestamp: "invalid date"}
      )

      assert {:ok, _} = Orders.amend(open_order, %{price: @amend_price, qty: @amend_qty})
    ...
    test "#{side} returns an error and records a failed transition when the order has an invalid status for pend amend" do
      ...
      assert {:error, reason} =
               Orders.amend(filled_order, %{price: @amend_price, qty: @amend_qty})
