defmodule Tai.TestSupport.Mocks.Responses.Orders.GoodTillCancel do
  ...
  alias Tai.Orders.{Order, Submissions, Responses}
  ...
  def create_accepted(venue_order_id, submission) do
    order_response = %Responses.CreateAccepted{
      id: venue_order_id,
  ...
  def amend_accepted(venue_order_id, match_attrs, response_attrs \\ %{}) do
    ...
    order_response = struct!(Responses.AmendAccepted, merged_response_attrs)
  ...
  @deprecated "Use amend_accepted/3 with venue_order_id instead."
  def amend_price_and_qty(%Order{} = order, price, qty) do
    amend_accepted(order.venue_order_id, %{price: price, qty: qty})
  end

  @deprecated "Use amend_accepted/3."
  def amend_price_and_qty(venue_order_id, price, qty) do
    amend_accepted(venue_order_id, %{price: price, qty: qty})
  end
  ...
  def amend_bulk_accepted(amend_set, response_attrs \\ []) do
    order_responses =
      ...
      |> Enum.map(fn {{order, _attrs}, idx} ->
        ...
        struct!(Responses.AmendAccepted, merged_response_attrs)
      end)

    response = %Responses.AmendBulk{orders: order_responses}
  ...
  def cancel_accepted(venue_order_id, attrs \\ %{}) do
    ...
    order_response = struct!(Responses.CancelAccepted, merged_attrs)
