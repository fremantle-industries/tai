defmodule Tai.TestSupport.Factories.NewOrderFactory do
  alias Tai.NewOrders.{Order, OrderCallback, OrderCallbackStore, OrderRepo}
  alias Tai.TestSupport.Factories.NewOrderSubmissionFactory
  ...
  def create_order_with_callback(attrs \\ %{}) do
    with {:ok, order} <- create_order(attrs) do
      order_callback = %OrderCallback{
        ...
        callback: NewOrderSubmissionFactory.fire_order_callback(self())
      }
