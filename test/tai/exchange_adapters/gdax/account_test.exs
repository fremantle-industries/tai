defmodule Tai.ExchangeAdapters.Gdax.AccountTest do
  ...
  test "order_status returns an error/message tuple when it can't find the order" do
    use_cassette "order_status_error" do
      assert Account.order_status(:my_gdax_exchange, "invalid-order-id") == {:error, "Invalid order id"}
    end
  end
  ...
  test "cancel_order returns an error tuple when it can't cancel the order" do
    use_cassette "cancel_order_error" do
      assert Account.cancel_order(:my_gdax_exchange, "invalid-order-id") == {:error, "Invalid order id"}
    end
  end
end
