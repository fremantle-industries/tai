...
defmodule Tai.Commands.HelperTest do
  ...
  test "markets displays all inside quotes and the time they were last processed and changed", %{test_feed_a_btcusd: test_feed_a_btcusd} do
    ...
    +-------------+--------+-----------+-----------+----------+----------+------------------+------------------+-----------------------+-----------------------+
    |        Feed | Symbol | Bid Price | Ask Price | Bid Size | Ask Size | Bid Processed At | Ask Processed At | Bid Server Changed At | Ask Server Changed At |
    +-------------+--------+-----------+-----------+----------+----------+------------------+------------------+-----------------------+-----------------------+
  ...
  test "orders displays items in ascending order from when they were enqueued" do
    ...
    +----------+--------+-------+------+--------+-----------+-----------+-------------+------------+
    | Exchange | Symbol | Price | Size | Status | Client ID | Server ID | Enqueued At | Created At |
    +----------+--------+-------+------+--------+-----------+-----------+-------------+------------+
    |        - |      - |     - |    - |      - |         - |         - |           - |          - |
    +----------+--------+-------+------+--------+-----------+-----------+-------------+------------+\n
    """
    ...
    |    Exchange | Symbol |    Price | Size |   Status |                            Client ID | Server ID | Enqueued At | Created At |
    +-------------+--------+----------+------+----------+--------------------------------------+-----------+-------------+------------+
    | test_feed_a | btcusd | 12999.99 |  1.1 | enqueued | #{btcusd_order.client_id           } |           |         now |            |
    | test_feed_b | ltcusd |    75.23 |  1.0 | enqueued | #{ltcusd_order.client_id           } |           |         now |            |
    +-------------+--------+----------+------+----------+--------------------------------------+-----------+-------------+------------+\n
    """
