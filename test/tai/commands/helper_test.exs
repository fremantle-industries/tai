...
defmodule Tai.Commands.HelperTest do
  ...
  test "markets displays all inside quotes and the time they were last processed and changed", %{test_feed_a_btcusd: test_feed_a_btcusd} do
    :ok = OrderBook.replace(
      ...
      %{
        bids: %{
          12999.99 => {0.000021, Timex.now, nil},
          12999.98 => {1.0, nil, nil}
        },
        asks: %{
          13000.01 => {1.11, Timex.now, nil},
    ...
    +-------------+--------+-----------+-----------+----------+----------+-------------------+-----------------+------------------+------------------+
    |        Feed | Symbol | Bid Price | Ask Price | Bid Size | Ask Size | Last Processed At | Last Changed At | Bid Processed At | Ask Processed At |
    +-------------+--------+-----------+-----------+----------+----------+-------------------+-----------------+------------------+------------------+
    | test_feed_a | btcusd |  12999.99 |  13000.01 | 0.000021 |     1.11 |                   |                 |              now |              now |
