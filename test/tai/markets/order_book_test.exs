defmodule Tai.Markets.OrderBookTest do
  ...
  describe ".replace" do
    ...
    test "broadcasts a system event", %{book_pid: book_pid} do
      ...
      assert OrderBook.replace(book_pid, snapshot) == :ok
  ...
  describe ".bids" do
    test "returns a full price ordered list", %{book_pid: book_pid} do
      :ok =
        OrderBook.replace(book_pid, %OrderBook{
          bids: %{
            146.00 => {10.1, nil, nil},
    ...
    test "can limit the depth returned", %{book_pid: book_pid} do
      :ok =
        OrderBook.replace(book_pid, %OrderBook{
          bids: %{
            146.00 => {10.1, nil, nil},
  ...
  describe ".bid" do
    test "returns the first price point", %{book_pid: book_pid} do
      :ok =
        OrderBook.replace(book_pid, %OrderBook{
          bids: %{
            146.00 => {10.1, nil, nil},
